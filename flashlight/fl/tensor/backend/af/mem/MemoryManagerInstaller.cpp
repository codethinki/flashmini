...
MemoryManagerAdapter* MemoryManagerInstaller::getImpl(
    ...
    return static_cast<MemoryManagerAdapter*>(ptr);
}
...
MemoryManagerInstaller::MemoryManagerInstaller(
    ...
    auto allocFn = [](
        ...
        MemoryManagerAdapter* m = MemoryManagerInstaller::getImpl(manager);
        try { *ptr = m->alloc(userLock, ndims, dims, elSize); }
            ...
            return static_cast<af_err>(AF_ERR_UNKNOWN);
        }
        // Log
        m->log(
            "alloc",
            /* size */
            dims[0],
            // HACK: dims[0] until af::memAlloc is size-aware
            userLock,
    ...
    auto allocatedFn = [](af_memory_manager manager, size_t* size, void* ptr) {
        MemoryManagerAdapter* m = MemoryManagerInstaller::getImpl(manager);
        m->log("allocated", reinterpret_cast<std::uintptr_t>(ptr));
    ...
    auto unlockFn = [](af_memory_manager manager, void* ptr, int userLock) {
        MemoryManagerAdapter* m = MemoryManagerInstaller::getImpl(manager);
        m->log("unlock", reinterpret_cast<std::uintptr_t>(ptr), userLock);
        m->unlock(ptr, static_cast<bool>(userLock));
        return AF_SUCCESS;
    };
    ...
    auto userLockFn = [](af_memory_manager manager, void* ptr) {
        MemoryManagerAdapter* m = MemoryManagerInstaller::getImpl(manager);
        m->log("userLock", reinterpret_cast<std::uintptr_t>(ptr));
        m->userLock(ptr);
        return AF_SUCCESS;
    };
    ...
    auto userUnlockFn = [](af_memory_manager manager, void* ptr) {
        MemoryManagerAdapter* m = MemoryManagerInstaller::getImpl(manager);
        m->log("userUnlock", reinterpret_cast<std::uintptr_t>(ptr));
    ...
    auto isUserLockedFn = [](af_memory_manager manager, int* out, void* ptr) {
        MemoryManagerAdapter* m = MemoryManagerInstaller::getImpl(manager);
        m->log("isUserLocked", reinterpret_cast<std::uintptr_t>(ptr));
    ...
    AF_CHECK(
        af_memory_manager_set_jit_tree_exceeds_memory_pressure_fn(
            itf,
            jitTreeExceedsMemoryPressureFn
        )
    );
    ...
    auto nativeAllocFn = [itf](size_t const bytes) {
        void* ptr;
...
MemoryManagerAdapter* MemoryManagerInstaller::currentlyInstalledMemoryManager() {
