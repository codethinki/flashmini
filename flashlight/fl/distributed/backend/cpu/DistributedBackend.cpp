...
void allReduce(fl::Tensor& tensor, bool async /* = false */) {
    ...
    if(tensorSize > cacheTensor_.elements())
        cacheTensor_ =
            fl::Tensor({static_cast<Dim>(tensorSize)}, fl::dtype::b8);
    ...
    switch(tensor.type()) {
        case fl::dtype::f32:
            detail::allreduceGloo(
                static_cast<float*>(cacheTensorPtr.get()),
                tensor.elements()
            );
            break;
        case fl::dtype::f64:
            detail::allreduceGloo(
                static_cast<double*>(cacheTensorPtr.get()),
                tensor.elements()
            );
            break;
        case fl::dtype::s32:
            detail::allreduceGloo(
                static_cast<int*>(cacheTensorPtr.get()),
                tensor.elements()
            );
            break;
        case fl::dtype::s64:
            detail::allreduceGloo(
                static_cast<int64_t*>(cacheTensorPtr.get()),
                tensor.elements()
            );
            break;
        default:
