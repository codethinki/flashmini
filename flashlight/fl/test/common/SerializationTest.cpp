...
struct Basic {
    ...
    FL_SAVE_LOAD(x, y, s, v) Basic() = default;

    Basic(int x, double y, std::string s, std::vector<int> v) : x(x),
                                                                y(y),
                                                                s(std::move(s)),
                                                                v(std::move(v)) {}
...
struct BasicV1 {
    ...
    FL_SAVE_LOAD(x, y, fl::versioned(z, 1), s, v) BasicV1() = default;

    BasicV1(int x, double y, float z, std::string s, std::vector<int> v) : x(x),
                                                                           y(y),
                                                                           z(z),
                                                                           s(std::move(s)),
                                                                           v(std::move(v)) {}
...
struct NestedVersioned {
    ...
    FL_SAVE_LOAD(
        ...
        fl::versioned(fl::versioned(w, 0), 0, 1)
    ) /* 2 \not\in [0, 1] */
};
