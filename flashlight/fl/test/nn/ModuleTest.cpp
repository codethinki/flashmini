...
TEST(ModuleTest, EmbeddingFwd) {
    ...
    auto wtVar = param(
        Tensor::fromVector<float>(
            {embDim, nEmb},
            {8, 2, 2, 10, 5, 3, 3, 4, 6, 12, 3, 8, 0, 5, 2}
        )
    );
...
TEST(ModuleTest, RNNFwd) {
    ...
    auto in = Variable(
        fl::rand({input_size, batch_size, seq_length}, fl::dtype::f32),
        true
    );
...
TEST_F(ModuleTestF16, RNNFwdF16) {
    ...
    auto in = Variable(
        fl::rand({input_size, batch_size, seq_length}, fl::dtype::f16),
        true
    );
    unsigned n_params = 51;
...
TEST(ModuleTest, DropoutFwd) {
    ...
    ASSERT_GT(
        fl::amax(out.tensor()).scalar<float>(),
        1.5
...
TEST(ModuleTest, AdaptiveSoftMaxLossBatchFwd) {
    ...
    for(int i = 0; i < B; ++i) {
        auto singleOutVar = asml->forward(
            x(fl::span, fl::span, fl::range(i, i + 1)),
            y(fl::span, fl::range(i, i + 1))
        );
...
TEST(ModuleTest, ModuleCloneCopy) {
    ...
    ASSERT_TRUE(
        allClose(
            clonedModulePtr->forward({inVar}).front(),
            expected_outVar,
            1E-7
        )
    );
