...
TEST(TensorReductionTest, argmin) {
    ...
    ASSERT_EQ(
        fl::argmin(in, 0, /* keepDims = */ true).shape(),
        Shape({1, in.dim(1)})
    );
    ASSERT_EQ(
        fl::argmin(in, 1, /* keepDims = */ true).shape(),
        Shape({in.dim(0), 1})
    );
}
...
TEST(TensorReductionTest, argmax) {
    ...
    ASSERT_EQ(
        fl::argmax(in, 0, /* keepDims = */ true).shape(),
        Shape({1, in.dim(1)})
    );
    ASSERT_EQ(
        fl::argmax(in, 1, /* keepDims = */ true).shape(),
        Shape({in.dim(0), 1})
    );
}
...
TEST(TensorReductionTest, mean) {
    ...
    ASSERT_EQ(
        fl::mean(r, {0, 1}, /* keepDims = */ true).shape(),
        Shape({1, 1, 6})
    );
