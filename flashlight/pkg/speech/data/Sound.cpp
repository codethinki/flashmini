...
static sf_count_t sf_vio_ro_seek(sf_count_t offset, int whence, void* user_data) {
    std::istream* f = reinterpret_cast<std::istream*>(user_data);
...
static sf_count_t sf_vio_wo_seek(sf_count_t offset, int whence, void* user_data) {
    std::ostream* f = reinterpret_cast<std::ostream*>(user_data);
    std::ios_base::seekdir way;
    switch(whence) {
        case SEEK_CUR:
...
static sf_count_t sf_vio_wo_read(void* /* ptr */, sf_count_t /* count */, void* /* user_data */) {
    throw std::invalid_argument("write-only stream");
    return 0;
}

static sf_count_t sf_vio_wo_write(const void* ptr, sf_count_t count, void* user_data) {
    std::ostream* f = reinterpret_cast<std::ostream*>(user_data);
    auto pos = f->tellp();
