...
private:
    ...
    ctcStatus_t launch_alpha_beta_kernels(
    ...
    ctcStatus_t launch_gpu_kernels(
        const ProbT* const probs,
        ProbT* grads,
        size_t config,
        bool launch_alpha,
    ...
    ctcStatus_t setup_gpu_metadata(
        const int* const flat_labels,
        const int* const label_lengths,
        const int* const input_lengths
    );
    ...
    ctcStatus_t compute_log_probs(const ProbT* const activations);
...
template<typename ProbT>
ctcStatus_t GpuCTC<ProbT>::setup_gpu_metadata(
    const int* const flat_labels,
    const int* const label_lengths,
    const int* const input_lengths
) {
...
template<typename ProbT>
template<int NT, int VT>
ctcStatus_t GpuCTC<ProbT>::launch_alpha_beta_kernels(
    const ProbT* const probs,
    ProbT* grads,
    bool compute_alpha,
    bool compute_beta
...
template<typename ProbT>
ctcStatus_t GpuCTC<ProbT>::compute_log_probs(const ProbT* const activations) {
...
template<typename ProbT>
ctcStatus_t GpuCTC<ProbT>::cost_and_grad(
    const ProbT* const activations,
    ProbT* grads,
    ProbT* costs,
    const int* const flat_labels,
...
template<typename ProbT>
ctcStatus_t GpuCTC<ProbT>::score_forward(
    const ProbT* const activations,
    ProbT* costs,
    const int* const flat_labels,
    const int* const label_lengths,
