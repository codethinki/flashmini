...
private:
    ...
    std::tuple<ProbT, bool> cost_and_grad_kernel(
        ProbT* grad,
...
template<typename ProbT>
int CpuCTC<ProbT>::CpuCTC_metadata::setup_labels(
    const int* const labels,
    int blank_label,
    int L,
    int S
...
std::tuple<ProbT, bool> CpuCTC<ProbT>::cost_and_grad_kernel(
    ...
    ProbT llForward = compute_alphas(
        probs,
        ctcm.repeats,
        S,
        T,
...
ProbT CpuCTC<ProbT>::compute_betas_and_grad(
    ...
    for(int i = 0; i < alphabet_size_; ++i) {
        ...
        if(
            output[i] == 0.0 || output[i] == ctc_helper::neg_inf<ProbT>()
            || probs[idx3] == 0.0
        ) {
            grad[idx3] = probs[idx3];
        } else {
            grad[idx3] = probs[idx3] - std::exp(
                output[i]
