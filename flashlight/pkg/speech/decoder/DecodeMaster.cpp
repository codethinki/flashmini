...
std::pair<std::vector<int64_t>,
    std::vector<int64_t>> DecodeMaster::computeMetrics(const std::shared_ptr<fl::Dataset>& predDataset) {
...
std::shared_ptr<fl::lib::text::Trie> DecodeMaster::buildTrie(
    ...
    auto trie = std::make_shared<fl::lib::text::Trie>(
        tokenDict_.indexSize(),
        tokenDict_.getIndex(trainOpt_.wordSep)
    );
...
std::shared_ptr<fl::Dataset> DecodeMaster::decode(
    ...
    for(auto& sample : *emissionDataset) {
        ...
        tokensV.erase(
            std::remove(tokensV.begin(), tokensV.end(), -1),
            tokensV.end()
        );
