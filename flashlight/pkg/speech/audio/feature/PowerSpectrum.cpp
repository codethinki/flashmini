...
PowerSpectrum::PowerSpectrum(const FeatureParams& params) : featParams_(params),
                                                            dither_(params.ditherVal),
                                                            preEmphasis_(params.preemCoef,
                                                                params.numFrameSizeSamples()),
                                                            windowing_(params.numFrameSizeSamples(),
                                                                params.windowType) {
    ...
    fftPlan_ = std::make_unique<fftw_plan>(
        std::move(
            fftw_plan_dft_r2c_1d(
                nFFt,
                inFftBuf_.data(),
                (fftw_complex*) outFftBuf_.data(),
                FFTW_MEASURE
            )
        )
    );
